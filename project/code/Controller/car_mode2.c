...
static float car_mode2_normalize_signed_lamp_angle_deg(float angle_deg)
{
...
static void car_mode2_calc_ref_from_lamp(float cx, float cy, float angle_deg,
                                         float *ref_x, float *ref_y)
{
    const float angle_rad =
        car_mode2_normalize_signed_lamp_angle_deg(angle_deg) * MODE2_DEG_TO_RAD;
    ...
    *ref_x = cx - (MODE2_CAR_LAMP_OFFSET_PX * sinf(angle_rad));
    *ref_y = cy + (MODE2_CAR_LAMP_OFFSET_PX * cosf(angle_rad));
}

static void car_mode2_rotate_air_to_car(float air_x, float air_y,
                                        float angle_deg,
...
static uint8 car_mode2_angle_position_allowed(float ref_x, float ref_y)
{
    ...
    target_x = g_air_mode2_target_x - ref_x;
    target_y = g_air_mode2_target_y - ref_y;
...
static uint8 car_mode2_car_position_allowed(void)
{
    ...
    g_car_mode2_state.car_position_valid =
        (g_air_mode2_car_lamp_valid > 0.5f) ? 1U : 0U;

    if(g_car_mode2_state.car_position_valid == 0U)
    {
    ...
    car_mode2_calc_ref_from_lamp(g_air_mode2_car_lamp_cx,
...
void car_mode2_update_100HZ(uint32 now_ms)
{
    ...
    g_car_mode2_state.target_valid =
        (g_air_mode2_target_valid > 0.5f) ? 1U : 0U;
    ...
    if(g_car_mode2_state.target_valid != 0U)
    {
        const float raw_delta_x =
            g_air_mode2_target_x - g_car_mode2_state.car_position_x;
        const float raw_delta_y =
            g_air_mode2_target_y - g_car_mode2_state.car_position_y;

        car_mode2_rotate_air_to_car(raw_delta_x,
                                    raw_delta_y,
                                    g_air_mode2_lamp_angle_deg,
    ...
    {
        g_car_mode2_state.target_delta_x = 0.0f;
        g_car_mode2_state.target_delta_y = 0.0f;
    }
